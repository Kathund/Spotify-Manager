...
class PlaybackPauseRoute extends Route {
  ...
  async handle(req: Request, res: Response) {
    try {
      ...
      if (204 === currentPlayback.status || false === playback.playing) {
        return res.status(404).json({ success: false, cause: 'Nothing is playing.' });
      }
...
export default PlaybackPauseRoute;
